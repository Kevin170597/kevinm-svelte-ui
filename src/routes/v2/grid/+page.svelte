...
<div class="h-[100vh] overflow-auto p-8">
	<Flex s={{ direction: 'col', gap: 4 }}>
		<Title s={{ color: 'white', size: 'lg' }}>Grid</Title>
		<CodeBlock lang="ts" code={'import { Grid } from "sveltekev";'} radius="lg" />
		...
		<Grid s={{ gap: 4, color: dark ? 'black' : 'white'}}>
			{#each colors as color}
				{#each [1, 2, 3, 4, 5, 6] as colorLevel}
					<Grid item sz={2}>
                        <Grid s={{ color, colorLevel: colorLevel as 1 | 2 | 3 | 4 | 5 | 6 }}>1</Grid>
                    </Grid>
				{/each}
			{/each}
		</Grid>
